...
def get_base_folder() -> Path:
    """
    PyInstaller(onefile)で実行中は __file__ が一時展開先を指してズレることがあるため、
    exeの実体パス(sys.executable)を優先して基準フォルダを決める。
    """
...
def find_template(base: Path) -> Path:
    ...
    for p in templates:
        try:
            ...
            if TEMPLATE_SHEET in wb_tmp.sheetnames:
                return p
        except Exception:
            continue
...
def main():
    ...
    if not case_files:
        raise RuntimeError("caseMonth_*.csv が見つかりません。exeと同じフォルダに置いてください。")
    if not daily_files:
        raise RuntimeError("userCaseDaily_*.csv が見つかりません。exeと同じフォルダに置いてください。")

    out_path = base / "output.xlsx"
